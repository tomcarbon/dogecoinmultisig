...
(function () {
	...
	/* it's called version byte. thanks peoplma! */
	coinjs.pub = 0x1e;
	coinjs.priv = 0x9e;
...
//window.alert("Hello from top of coin.js");

	/* other vars */
	coinjs.developer = '9s9NHBgEDBrxhVQMbPgUbdQEKPVGjcfeiG'; // dogecoin
	...
	*/
	...
	coinjs.script = function(data) {
		...
		r.spendToScript = function(address){
			...
			var s = coinjs.script(); 
			if(addr.version==coinjs.multisig){ // multisig address TCCDEBUG 20160730 was hard-coded 5
	...
	coinjs.transaction = function() {
		...
		r.addoutput = function(address, value){
			var o = {};
console.info("here we are in r.addoutput. address = " + address);
...
/***************************************
* is this gold? 
* source: http://stackoverflow.com/questions/3583724/how-do-i-add-a-delay-in-a-javascript-loop
**************************************/
	...
	/**************************************************************************
	* A moist Randomizer 
	* randomizer with human-added randomness factor 
	* returns string randomized character from bank of 62 values.
	* input provides length for this string (64 usu)
	* tcc20160829
	* original source: coinjs.random
	**************************************************************************/
	coinjs.random = function(length) {
		var human_style = true;		
		if (human_style == true)	{	
			/* randomizer with human-provided randomness factor */
			var r = "";
			var l = length || 25;
			...
			for(x=0;x<l;x++) {
	  		...
	  		var tempstring = $("#userRandom").val();		// grab the user specified random string from the site

			/* if the human powered string is zero length we can return now. */
			if (tempstring.length == 0) {
				console.info("No human supplied data was found. So not using it");
				return r;
			}

			var r2 = "";
			for (i=0; i<r.length;i++) {
				/* is a ~randomly selected human number even or odd? */
				var a0 = ((tempstring.charCodeAt((Math.random() * tempstring.length))%2) == 0 ? "even" : "odd"); 

				/* it's approx a 50% chance that we'll actually use the human element */
				if (a0 == "odd") {
					/* do nothing special */
					r2 += r.charAt(i);		// just use the random looking original character.
				} else {	
					var a1 = r.charCodeAt(i);		// here is the previously computed element, converted to ascii */

					/* take another ~randomly selected character from the human array, converted to ascii*/
					var a2 = tempstring.charCodeAt((Math.random() * tempstring.length));

					/* add the ascii values together, then multiply them by a decent-sized prime number. */
					/* ... then take the remainder to get a number between 0 and 61 */
					var a3 = ((a1 + a2) * 41729)%62;
					r2 += chars.charAt(a3);		// same as above, added some free will (whtvrs)
				}
			}
			//console.info("past myLoop logic. r = " + r + " and r2 = " + r2);
			return r2;
		} 
	}
