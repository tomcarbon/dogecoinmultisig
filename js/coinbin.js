$(document).ready(function() {
	...
	$("#newMultiSigAddress").click(function(){
		...
		/* perform duplicate checking of the pubkeys new 20160822 */
		console.info("Validation: Checking the pubkeys for duplicates...");
		for (var i=0;i<keys.length;i++) {
			for (var j=0; j<keys.length;j++) {
				if (i == j) {	
					...
					if (keys[i] == keys[j]) {
						var tt1 = "ERROR: Duplicate PubKeys found for this new Multisig attempt... This is bad. \n" + 
								"The Public Key of '" + keys[i] + "' exists in indexes " + i + " and " + j + ".\n" + 
								"FAIL. Will not proceed further (you should start over and try again).";
						console.error(tt1);
						alert(tt1);	
						return false;	// this is an ERROR and we will NOT create a new Multisig this time.
					}
				}
			}
		}
		console.info("Validation complete. No duplicate pubkeys were found.");

		var sortKeysCheckBox = document.getElementById("c1").checked;
		if (sortKeysCheckBox == true)	 {			// we will do the sort here
			console.info("The pubkeys WILL be sorted from their original placements.");
			keys.sort();
		} else {
			console.info("The pubkeys will NOT be sorted from their original placements.");
		}
		
		/* new for the sorted pubkeys */
		for (var i=0;i<keys.length;i++) {
			console.info("Index(" + i + ");" + keys[i]);	
			if(coinjs.pubkeydecompress(keys[i])){
				$("#multisigPubKeys .pubkey").parent().removeClass('has-error');
			} else {
				$("#multisigPubKeys .pubkey").parent().addClass('has-error');
			}
		}
/*
		$.each($("#multisigPubKeys .pubkey"), function(i,o){
...
//		if($("#multisigPubKeys .pubkeyRemove").length<14){
		if($("#multisigPubKeys .pubkeyRemove").length<2){
	...
	$("#transactionBtn").click(function(){		// tempola I put a 3 there.
		var tx = coinjs.transaction();
	...
	function redeemingFrom(string){
		...
		if(decode.version == coinjs.pub){ // regular address
			...
			r.isMultisig = false;
			alert("TCCDEBUG: yo this should be true");
		} else {
			var script = coinjs.script();
			var decodeRs = script.decodeRedeemScript(string);
alert("TCCDEBUG:309seg");
...
alert("milky");
	...
	function rawSubmitBlockrio_litecoin(thisbtn){ 
		...
		$.ajax ({
			type: "POST",
			url: "https://doge.blockr.io/api/v1/tx/push",
			data: {"hex":$("#rawTransaction").val()},
			dataType: "json",
			error: function(data) {
				var obj = $.parseJSON(data.responseText);
	...
	$("#signBtn").click(function(){
		...
		$("#signPleaseWaitBox").removeClass('hidden');
		//alert("PITA callback substitute for menu Sign option. This is not an error.");
		setTimeout(function(){
			...
			$("#signPleaseWaitBox").addClass('hidden');
		}, 500);
	});
...
//	$("#newKeysBtn, #newHDKeysBtn").click();    // addition randomization logic removed 20160828
	...
	$(".qrcodeBtn").click(function(){
		...
		if(qrstr){
			var tempqrstr = qrstr.trim();
			qrcode.makeCode(tempqrstr);
